...
def upload_image():
    ...
    if file and allowed_file(file.filename):
        ...
        # Find snake details
        selected = next(
            (snake for snake in snakes_data if snake["scientific_name"].lower() == predicted_class.lower()), 
            None
        )

        if selected is None:
            return jsonify({"msg": "Snake not found in database"}), 404

        selected = selected.copy()  # Make a copy to avoid modifying the original

        selected['imageUrl'] = url_for('uploaded_file', filename=filename)
        selected['pre-hospital_measures'] = non_ven if selected["venomous"] == "Non-venomous" else ven
...
database_dir = './training_set/training_set/dogs'

# Load the database images directly from the directory
database_images = [img_file for img_file in os.listdir(database_dir)]
loaded_features = np.load('database_features.npy')
...
def display_images(input_image_path, matched_image_paths):
    ...
    fig, axes = plt.subplots(1, 5, figsize=(20, 5))  # 1 input image + 4 matched images
    axes[0].imshow(input_img)
    axes[0].set_title('Input Image')
    axes[0].axis('off')
    
    for i, matched_image_path in enumerate(matched_image_paths):
        matched_img = image.load_img(matched_image_path)
        axes[i + 1].imshow(matched_img)
        axes[i + 1].set_title(f'Matched Image {i + 1}')
        axes[i + 1].axis('off')
    
    # Save the figure
    plt.savefig('static/matched_images.png')
...
def upload_file():
    ...
    if file:
        ...
        # Process the file (dummy processing, just renaming here)
        model = ResNet50(weights='imagenet', include_top=False, pooling='avg')

            # Extract features of the uploaded image
        input_image_features = extract_features(file_path, model)

            # Fit the k-NN model to the loaded database image features
        knn = NearestNeighbors(n_neighbors=4, metric='euclidean')
        knn.fit(loaded_features)

            # Find the closest matches in the database
        distance, indices = knn.kneighbors([input_image_features])

            # Get the matched image filenames
        print("Indices = ",indices[0])
        matched_image_paths = [os.path.join(database_dir, database_images[i]) for i in indices[0]]

            # Display the input image and the matched images side by side
        display_images(file_path, matched_image_paths)
        img_url = url_for(UPLOAD_FOLDER, filename='matched_images.png',_external = True)
